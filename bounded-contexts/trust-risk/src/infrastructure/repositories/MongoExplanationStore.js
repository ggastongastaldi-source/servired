...
/**
 * MongoExplanationStore — colección append-only de Explanations.
 * ADR-008: toda decisión de scoring debe ser explicable y auditable.
 * GIA lee desde aquí sin reconstruir lógica de dominio.
 */
class MongoExplanationStore extends IExplanationStore {

  constructor(db) {
    super();
    this._col = db.collection('trust_explanations');
  }
  ...
  async getByEvidenceId(evidenceId) {
    return this._col.findOne({ evidenceId }) || null;
  }
}
