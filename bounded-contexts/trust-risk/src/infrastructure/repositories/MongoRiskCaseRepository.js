...
class MongoRiskCaseRepository extends IRiskCaseRepository {
  ...
  constructor(eventStore, db) {
    ...
    this._index      = db.collection('risk_case_index');
  }
  ...
  async save(riskCase) {
    ...
    await this._eventStore.append(
      riskCase.id,
      events,
      riskCase.expectedVersion
    );

    // Actualizar índice de búsqueda rápida
    await this._index.updateOne(
      { riskCaseId: riskCase.id },
      { $set: {
        riskCaseId:     riskCase.id,
        trustProfileId: riskCase.trustProfileId,
        status:         riskCase.status,
        severity:       riskCase.severity ? riskCase.severity.value : null,
        updatedAt:      new Date(),
      }},
      { upsert: true }
    );
