...
function makeUow(repo) {
  return new UnitOfWork(repo, () => {});
}

const CMD_BASE = {
  clienteId: 'cliente-001', tipoServicio: 'electricidad',
  zona: 'AMBA-SUR', precio: 8000, pagoWorker: 5500,
};
...
async function run() {
  ...
  const r1   = await new CrearPedido(uow).execute(CMD_BASE);
  ...
  const r2 = await new AsignarWorker(uow).execute({ pedidoId: r1.pedidoId, workerId: 'worker-007' });
  ...
  const uow2  = makeUow(repo2);
  const r3    = await new CrearPedido(uow2).execute(CMD_BASE);
  const r4    = await new CancelarPedido(uow2).execute({ pedidoId: r3.pedidoId, motivo: 'test' });
  ...
  const uow3  = makeUow(repo3);
  const r5    = await new CrearPedido(uow3).execute(CMD_BASE);
  const pid   = r5.pedidoId;

  await new AsignarWorker(uow3).execute({ pedidoId: pid, workerId: 'w1' });
  const p = await repo3.findById(pid);
  p.iniciarTrabajo();
  p.finalizarTrabajo();
  await repo3.save(p);

  const r6 = await new LiberarPago(uow3).execute({ pedidoId: pid });
  ...
  await assert.rejects(
    () => new AsignarWorker(uow3).execute({ pedidoId: 'no-existe', workerId: 'w' }),
    /no encontrado/
  );
