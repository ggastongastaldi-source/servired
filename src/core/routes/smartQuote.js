...
const PRECIOS = {
  ...
  servicio_domestico:{ baja: 7500,    alta: 12000   }, // precio POR HORA ARS
...
router.get('/watch/precio', async (req, res) => {
  ...
  let base;
  let unidad = 'hora';
  let fuente = 'aladdin-mongo';

  try {
    const vivo = await PrecioMercado.findOne({ rubro: r.toLowerCase() }).lean();
    if (vivo) {
      base = vivo.baja;
      unidad = vivo.unidad || 'hora';
    } else {
      base = PRECIOS[r]?.baja || 85000;
      fuente = 'static-fallback';
      console.log(`[SmartQuote] ⚠️ Rubro ${r} no en DB, usando fallback.`);
    }
  } catch (err) {
    console.error('[SmartQuote] ❌ Error DB:', err.message);
    base = PRECIOS[r]?.baja || 90000;
    fuente = 'emergency-hardcoded';
  }

  const base_usado = base;
  ...
  const total = Math.round(base_usado * mult * hrs);
  console.log(`[SmartQuote] 💰 ${r} | $${total} | fuente: ${fuente}`);
  res.json({ r, total, unidad, worker: Math.round(total * 0.8), srv: Math.round(total * 0.2), f: fuente });
});
