...
router.post('/', async (req, res) => {
  const { messages, appMode, correlationId } = req.body;

  if (!messages || !Array.isArray(messages)) {
    return res.status(400).json({ error: 'messages requerido' });
  }

  // Contexto de rol inyectado como mensaje de sistema adicional
  const roleContext = appMode
    ? `\n\nContexto actual: el usuario está en modo "${appMode}".`
    : '';
